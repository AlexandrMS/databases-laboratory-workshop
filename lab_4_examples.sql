...
SELECT TRIM(CONCAT_WS(' ', Фамилия, Имя, Отчество)) AS ФИО, Дата_рождения, Специализация, Год_присвоения_звания
FROM 
	Академики 
...
SELECT 
	...
	CHARINDEX('о', CONCAT_WS(' ', Фамилия, Имя, Отчество)) AS Позиция_о
...
SELECT 
	CONCAT_WS(' ', Фамилия, Имя, Отчество) AS ФИО,
	REVERSE(CONCAT_WS(' ', Фамилия, Имя, Отчество)) AS ФИО_Обр
FROM 
	Академики 
...
/* Пример 12: : Вывести наименьшее целое число, которое больше или равно 123.456, и
наибольшее целое число, которое меньше или равно 123.456 */
SELECT 
...
/* Пример 17: Академик Игорь Евгеньевич Тамм родился 8 июля 1895 года. И.Е. Тамм
скончался 12 апреля 1971 года. Вывести количество прожитых дней: */ 
SELECT
