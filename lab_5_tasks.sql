...
FROM 
	Страны 
WHERE 
	Название LIKE '%ан' AND Название NOT LIKE '%стан'
...
SELECT 
	CAST(MAX(Площадь) AS FLOAT) / MIN(Площадь) AS Разница_площадей 
FROM 
	Страны
...
SELECT 
	Континент, 
	FLOOR(SUM(Население) * 1.1) AS Прогноз_населения 
FROM 
	Страны 
GROUP BY 
	Континент
...
FROM 
	Страны 
GROUP BY 
	Континент HAVING MAX(Площадь) <= 10000 * MIN(Площадь)

/* 11. Вывести среднюю длину названий Африканских стран. */
SELECT 
	AVG(CAST(LEN(Название) AS FLOAT)) AS Средняя_длина 
FROM 
	Страны 
WHERE 
	Континент = 'Африка'
...
WHERE 
	Население > 1000000 
GROUP BY 
	Континент HAVING AVG(CAST(Население AS FLOAT) / Площадь) > 30